                            ...
                            data.length < 1 && props.type === "sales" ? <p className="text-xl my-10">لا يوجد مبيعات</p> :
                                data.length < 1 && props.type !== "sales" ? <p className="text-xl my-10">لا يوجد منتجات</p> :
                                    data?.map((product) => {
                                        return (
                                            props.type === "sales" ?
                                                <li key={product.id} className={`flex my-2 justify-between bg${props.secondry} text-[15px] px-2 py-1 rounded-md`}><h4>{product.sold_number}</h4><p>{product.total} SR</p></li>
                                                :
